...
class Controller:
    ...
    def run_policy(self):
        ...
        ############################################ NAVIGATION ############################################
            ...
            self.locomotion_vel_command[0] = 0.06
            self.locomotion_vel_command[1] = 0.0
            self.locomotion_vel_command[2] = np.clip(heading_error, -0.5, 0.5)
            
            if self.stop_locomotion:
                self.locomotion_vel_command = np.array([0., 0., 0.])
                phase = 0.0

            if self.navigation_counter > 500:
                self.locomotion_vel_command[0] = 0.0
                self.locomotion_vel_command[2] = np.clip(2*heading_error, -0.5, 0.5)
                if np.linalg.norm(self.locomotion_vel_command) < 0.05:
