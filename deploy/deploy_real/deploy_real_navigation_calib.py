...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        self.tag_poses_from_cam = np.zeros((0, 7))
    ...
    def run_policy(self):
        ...
        ################################# EETrack #################################
            ...
            # print(self.target_pose_b)torso_link
    ...
    def dump_observations_and_joint_pos_target(self, target_dof_pos):
        ...
        if self.task == "sit":
            ...
            try:
                r_wrist_yaw_pos_b, r_wrist_yaw_quat_b = body_pose(self.tf_buffer, "right_wrist_yaw_link", "pelvis", rot_type="quat")
                self.wrist_yaw_poses_b_gt = np.vstack((self.wrist_yaw_poses_b_gt, np.concatenate((r_wrist_yaw_pos_b, r_wrist_yaw_quat_b))))
                
                tag_pos_cam, tag_quat_cam = body_pose(self.tf_buffer, "target_arm", "zed2i_left_camera_optical_frame", rot_type="quat")
                self.tag_poses_from_cam = np.vstack((self.tag_poses_from_cam, np.concatenate((tag_pos_cam, tag_quat_cam))))
            except:
                print("Tag is not detected !!!!!!!!!!!")
    ...
    def log_metrics_and_trajectories(self):     
        ...
        trajectories = {
            ...
            "wrist_yaw_poses_b_gt": self.wrist_yaw_poses_b_gt,
            "tag_poses_from_cam": self.tag_poses_from_cam
