...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        # self.eetrack_obsmap = us.EETrackObservationWithLastAction(config, self.tf_buffer
        # )
    ...
    def run_policy(self):
        ...
        if self.task == "locomotion":
            curr_keymap = self.remote_controller.button[KeyMap.X] == 1
            if curr_keymap:
                self.stop_locomotion = True
            
            if self.locomotion_velocity_command is None:
                self.locomotion_velocity_command = ul.LocomotionVelocityCommand()
            if self.locomotion_phase_command is None:
                self.locomotion_phase_command = ul.LocomotionPhaseCommand(
                    period=0.8,
                )
            velocity_command = self.locomotion_velocity_command(quat_wxyz, self.stop_locomotion, self.counter)
            phase_command = self.locomotion_phase_command(self.counter)

            self.obs = self.locomotion_obsmap(self.low_state, velocity_command, phase_command, self.locomotion_last_action)
            obs_tensor = th.from_numpy(self.obs).unsqueeze(0)
            obs_tensor = obs_tensor.detach().clone().float()
            self.locomotion_action = self.locomotion_policy(obs_tensor).detach().numpy().squeeze() # TODO

            self.locomotion_last_action = self.locomotion_action.copy()
            
            locomotion_target_dof_pos = self.locomotion_actmap(self.locomotion_action) # TODO
            target_dof_pos = locomotion_target_dof_pos
    ...
    def run_wrapper(self):
        if self.mode == Mode.wait:
            ...
            if self._mode_change:
                ...
                print("[Locomotion] Press Button {X} to stop locomotion.")
                print("-------------------------------------------------")
                print("[Sit] Press Button {B} to change task to Sitting.")
                print("-------------------------------------------------")
