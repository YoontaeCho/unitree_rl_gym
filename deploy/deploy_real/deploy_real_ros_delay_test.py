...
class Controller:
    def __init__(self, config: Config) -> None:
        ...
        self.num_motors = len(self.config.motor_joint)
        
        self.step_magnitude = [0.3]   # TODO: tune this!!!!
        self.num_test = len(self.step_magnitude)
        ...
        # (0: move forward, 1: move backward, 2: next step)
        self.state = 0

        self.motor_movement = {'real_tick': [], 'real_movement': [], 'goal_tick': [], 'goal_movement': []}
    ...
    def run_step(self):
        ...
        if self.state == 0:
            self.offset = self.step_magnitude[self.test_idx]
            if elapsed_in_state >= 8000:
                self.state = 1
                self.state_start_tick = current_tick
            ...
            if elapsed_in_state >= 10:  
                self.iteration_idx += 1
                
                if self.iteration_idx < self.num_iteration:
                    self.state = 0
                else:
                    self.test_idx += 1
                    self.iteration_idx = 0
                    
                    if self.test_idx < self.num_test:
                        self.state = 0
                    else:
                        filename = f"{self.logpath}/motor_movement_{self.target_motor_idx}.json"
        ...
        q_target = self.q_initial.copy()
        q_target[self.target_motor_idx] += self.offset
    ...
    def run_wrapper(self):
        ...
        if self.mode == Mode.wait:
            ...
            if self._mode_change:
                print("Run test for step response.")
                self._mode_change = False
                self.counter = 0
            self.run_step()
